...
rawData <- readr::read_csv("data/Turkey-Vulture-Leo.csv")
...
pks <- findpeaks(c(a[[aBandNumber]]), minpeakheight = minHeight, minpeakdistance = minDist, plot = TRUE, npeaks = npeaks)
```
...
```{r}
# Take the peak with as definition for the segments,
# pks[ ,5] is the left border and pks[ ,6] the right on,
# at half the prominence height.
segLims <- Reduce(c, apply(X = pks, MARGIN = 1, FUN = function(X){c(X[5], X[6])}))
segLims <- c(segLims, length(sig))
nSegs <- length(segLims)
segmentNum <- nSegs
segments <- numeric(tail(segLims,1))
for (i in nSegs:1) {
  segments[1:segLims[i]] <- segmentNum
...
f <- c(0, as.numeric(diff(abs(det[,1])) > threshold))
id <- 1
segL <- do.call("rbind", lapply(X = 2:length(det), FUN = function(X){
  if (f[X] == 0) {
    c(det[X,1], id)
  } else if (f[X] == 1 && f[X] != f[X-1]) {
...
abline(v = which(c(0, diff(segL[,2]))==1), col = rgb(1, 0.6, 0, alpha = 0.3))
```
...
plot(det[,1],
     ...
     col = rgb(0.2, 0.6, 0.2),
     ylab = paste("Detail sub-band", dBandNumber))
abline(v = which(c(0, diff(segments[,3]))==1), col = rgb(1, 0.6, 0, alpha = 0.7))
```
